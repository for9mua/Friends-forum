    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="动态">
        <template #default="scope">
          <a target="_blank" :href="'/front/detail?id=' + scope.row.dynamicId" v-if="scope.row.dynamicId">{{ state.dynamicOptions.find(v => v.id === scope.row.dynamicId) ? state.dynamicOptions.find(v => v.id === scope.row.dynamicId).name : '' }}</a>
