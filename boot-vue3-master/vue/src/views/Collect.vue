...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="动态"><template #default="scope"><span v-if="scope.row.dynamicId">{{ state.dynamicOptions.find(v => v.id === scope.row.dynamicId) ? state.dynamicOptions.find(v => v.id === scope.row.dynamicId).name : '' }}</span></template></el-table-column>
      <el-table-column label="收藏人"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>
      ...
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item prop="dynamicId" label="动态">
          <el-select clearable v-model="state.form.dynamicId" placeholder="请选择"  style="width: 100%">
            <el-option v-for="item in state.dynamicOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
