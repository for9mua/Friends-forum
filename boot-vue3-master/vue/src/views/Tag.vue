...
const confirmDelBatch = () => {
  ...
  request.post('/tag/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/tag/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('tag.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 新增 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('tag.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/tag/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('tag.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('tag.deleteBatch')">
        <template #reference>
      ...
      <el-table-column prop="comments" label="备注"></el-table-column>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('tag.edit')">编辑</el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('tag.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="标签信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="comments" label="备注">
          <el-input v-model="state.form.comments" autocomplete="off"></el-input>
        </el-form-item>
